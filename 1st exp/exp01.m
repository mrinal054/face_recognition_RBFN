%exp01: 200 images of 40 persons for training dataset
%above 200 images are distorted for testing dataset
%------------------------------------------------
clc
clear all
format long;
...
dist=dis;    %%size(dist)=(400,40) %size(dist)=P*H
%distacne=sqrt(dist)
%-------------------------------------------------------------
%sigma calculation
...
G=g;
[row,coloum]=size(G); %size(G)=(400,40) %1st column means G1 %size(G)=P*H
%------------------------------------------------------------
%adding bias
for j=1:row 
    mm(j,:)=1;
